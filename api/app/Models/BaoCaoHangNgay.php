...
class BaoCaoHangNgay extends Model
{
    ...
    public function scopeTongSoGioLamNgay($query, $id, $thang)
    {
        return $query
            ->join('congviec', 'baocaohangngay.cv_id', '=', 'congviec.cv_id')
            ->select('congviec.cv_ten', 'baocaohangngay.bchn_giothamdinh', 'baocaohangngay.cv_id', 'baocaohangngay.bchn_ngay')
            ->where('baocaohangngay.nv_id', '=', $id)
            ->whereMonth('baocaohangngay.bchn_ngay', '=', $thang)
            ->sum('baocaohangngay.bchn_giothamdinh');
    }

    public function scopeSoGioLamNgay($query, $id, $thang)
    {
        ...
        foreach ($congviecvagio1 as $cv) {
            ...
            if (!isset($congviecvagio_moi[$cvId])) {
                $date = $cv->bchn_ngay;
                $dateTime = Carbon::parse($date)->format('d-m-Y');
        ...
        usort($congviecvagio_moi, function ($a, $b) {
            $dateTimeA = Carbon::createFromFormat('d-m-Y', $a['ngay-lam']);
            $dateTimeB = Carbon::createFromFormat('d-m-Y', $b['ngay-lam']);
            return $dateTimeA->greaterThan($dateTimeB) ? 1 : -1;
        });
        // Chuyển mảng kết hợp `congviecvagio_moi` thành mảng tuần tự
        $congviecvagio_moi = array_values($congviecvagio_moi);
